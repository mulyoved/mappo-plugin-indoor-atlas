...
function listener(success) {
    ...
    window.removeEventListener('indooratlas', listener, false);
}

var IndoorAtlas = {
    start: function start(success, error) {
        return window.addEventListener('indooratlas', function(){
            listener(success);
        }, false);
    }
};
...
require('cordova/exec/proxy').add('IndoorAtlas', IndoorAtlas);
